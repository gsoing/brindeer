...
public class ProfileController {
    ...
    public ResponseEntity<ProfileDto> createProfile(JwtAuthenticationToken principal) {
        ...
        profileDto.setLatitude((Float) principal.getTokenAttributes().get("latitude"));
        profileDto.setLongitude((Float) principal.getTokenAttributes().get("longitude"));
    ...
    @PutMapping(path = "/update_location_{id}", consumes = { MediaType.APPLICATION_JSON_VALUE })
    public ResponseEntity<ProfileDto> updateProfile(@PathVariable @NonNull String profileId,
                                                    @RequestBody @NonNull ProfileDto profileDto,
                                                    @PathVariable @NonNull Float profileLatitude,
                                                    @PathVariable @NonNull Float profileLongitude) {
        profileDto.setId(profileId);
        profileDto.setLatitude(profileLatitude);
        profileDto.setLongitude(profileLongitude);
        return ResponseEntity.ok(profileService.updateProfileLocation(profileDto.toModel()).toDto());
    }
